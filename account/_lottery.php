...
# Настройки лотерея
$amount_lottery = 3000; // Стоимость лотерейного билета
...
$_OPTIMIZATION["title"] = "Завершенные лотереи";
# список предыдущих лотерей
if(isset($_GET["winners"])){ ?>
<br/>
<table class='table table-bordered table-striped' cellpadding='3' align='center' width="99%">
	<tr>
...
Всего имеется <?=$num_bil; ?> билетов. После того, как все билеты будут проданы состоится розыгрыш счастливых билетов. Система случайным образом выберет 3 номера счастливых билетов и зачислит им призы. <BR />
<b>1 место - 50% от общего банка [<?=($amount_lottery * $num_bil) * 0.5; ?> монет]. <BR />
2 место - 25% от общего банка [<?=($amount_lottery * $num_bil) * 0.25; ?> монет]. <BR />
3 место - 20% от общего банка [<?=($amount_lottery * $num_bil) * 0.2; ?> монет].</b> <BR />
Остальные 5% составляют комиссию системы.
<BR />
<u>Стоимость билета = <?=$amount_lottery; ?> монет</u>.
<BR />
	...
	if(isset($_POST["set_lottery"], $_POST["hash"]) AND $_SESSION["lot_hash"] == $_POST["hash"]){
		...
		if($db->FetchRow() >= $amount_lottery){
			...
			if( $lid >= $num_bil){
				...
				echo "<center class='alert alert-success'>Лотерея окончена</center>";
				
			}else echo "<center class='alert alert-success'>Билет успешно куплен</center>";
			
		}else echo "<center class='alert alert-danger'>Недостаточно средств для покупки билета</center>";
...
<input type="submit" name="set_lottery" value="Купить билет" class="btn btn-success"  style="margin: 5px auto;" />
<input type="hidden" name="hash" value="<?=$_SESSION["lot_hash"]; ?>" />
</form>
</center>

<h4>Пользователи купившие билеты</h4>
<table class='table table-bordered' cellpadding='3' cellspacing='0' border='0' align='center' width="99%">
<thead>
	<th>№ билета</th>
	<th>Пользователь</th>
	<th>Дата</th>
